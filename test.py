...
for i, data_i in enumerate(dataloader):
    ...
    fake_keypoint, occlusion_pred, real_gray_map, fake_gray_map = model(data_i, mode='inference')
    _, source_gray_map = draw_pose_from_cords(
        data_i['source_keypoint'].detach().cpu() * torch.Tensor([opt.max_height - 1, opt.max_width - 1]),
    ...
    for b in range(fake_keypoint.shape[0]):
        ...
        visuals = dict()
        visuals['source_color_map'] = source_gray_map[b]
        visuals['fake_gray_map'] = fake_gray_map[b]
        visuals['real_gray_map'] = real_gray_map[b]
